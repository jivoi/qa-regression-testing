...
test_bind9() {
    ...
    if ! host -6 www.ubuntu.com ip6-localhost | grep "has address" ; then
        error="yes"
    fi
    if ! host -T -6 www.ubuntu.com ip6-localhost | grep "has address" ; then
        error="yes"
    fi
    if [ -z "$error" ]; then
        echo "PASS"
...
4. At this point the machine may boot into the initramfs (tested in 11.04) (if
   ...
   (either in initramfs or after login (--zero-superblock needed in 12.04, see LP: #943397)):
   # mdadm --zero-superblock /dev/vdb1
   # mdadm /dev/md0 -a /dev/vdb1
   # mdadm --zero-superblock /dev/vdb6
...
4. At this point the machine should boot into the initramfs (tested in 11.04) or
   ...
   # mdadm --zero-superblock /dev/vda6
   # mdadm /dev/md2 -a /dev/vda6
   # mdadm --zero-superblock /dev/vda5
...
test_virt() {
    echo "Testing virtualization server: "
    error=
    if ! dpkg -l ubuntu-virt-server | grep 'ii  ubuntu-virt-server' ; then
        error="yes"
    fi
...
case "$1" in
    ...
    virt)
        ...
        ;;
    *)
        echo "`basename $0` apache|bind9|cups|dovecot|lamp|lvm|elvm|mail|mysql|postfix|postgresql|raid1|samba|ssh <user@host>|tomcat|virt"
        exit 1
        ;;
esac
