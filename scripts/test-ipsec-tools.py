#!/usr/bin/python
#
#    test-ipsec-tools.py quality assurance test script for ipsec-tools
...
# QRT-Packages: ipsec-tools racoon

'''
    This script must be run in two virtual machines. Install required
    packages with:
        sudo apt-get -y install ipsec-tools racoon
    ...
    - 'psk' to test pre-shared key authentication
    - 'cert' to test x509 authentication
    - 'nat-t' to test nat-traversal (using a pre-shared key)

    ./test-ipsec-tools.py psk|cert|nat-t SELF_IP SELF_NET REMOTE_IP REMOTE_NET [OPTIONS]
    ...
    On system 10.0.0.10:
    sudo ./test-ipsec-tools.py psk 10.0.0.10 192.168.10.0/24 10.0.0.22 192.168.22.0/24 -v
...
class RacoonTest(testlib.TestlibCase):
    '''Test that racoon can set up an establish connections.'''
    ...
    def setUp(self):
        ...
        # Save, and turn on forwarding
        self.forwarding = open('/proc/sys/net/ipv4/ip_forward').read().strip()
    ...
    def tearDown(self):
        ...
        # Restore forwarding
        open('/proc/sys/net/ipv4/ip_forward','w').write('%s\n' % (self.forwarding))

    def test_00_configure(self):
        '''Configure racoon, SPD, interfaces, and routing'''

        # Stop daemon
        self.assertShellExitEquals(0,['/etc/init.d/racoon','stop'])
        self.assertShellExitEquals(0,['/etc/init.d/setkey','stop'])
        ...
        testlib.config_replace('/etc/ipsec-tools.conf','''# Test config
flush;
spdflush;

spdadd %s/%s[any] %s/%s[any] any -P in ipsec
   esp/tunnel/%s-%s/require;

spdadd %s/%s[any] %s/%s[any] any -P out ipsec
   esp/tunnel/%s-%s/require;
''' % (self.net_remote, self.netmask_remote, self.net_local, self.netmask_local,
       self.ip_remote, self.ip_local,
       self.net_local, self.netmask_local, self.net_remote, self.netmask_remote,
       self.ip_local, self.ip_remote))

        if test_type == 'psk':
            testlib.config_replace('/etc/racoon/racoon.conf','''# Test config
...
sainfo address %s/%s any address %s/%s any {
   pfs_group 2;
   encryption_algorithm aes;
...
''' % (self.ip_remote,
       ...
       self.net_remote, self.netmask_remote))
            ...
            testlib.config_replace('/etc/racoon/psk.txt','''# Test config
%s 0x21adf17a70f017e0e540e733cec62373f43b71290cf2eafe
''' % (self.ip_remote))
            os.chmod('/etc/racoon/psk.txt',0700)

        elif test_type == 'nat-t':
            testlib.config_replace('/etc/racoon/racoon.conf','''# Test config
path pre_shared_key "/etc/racoon/psk.txt";
...
remote %s {
   exchange_mode main;
   certificate_type x509 "hostacert.pem" "hostakey.pem";
...
''' % (self.ip_remote,
       ...
       self.net_remote, self.netmask_remote))
            ...
            testlib.config_replace('/etc/racoon/5a23788d.0',
'''-----BEGIN CERTIFICATE-----
...
-----END CERTIFICATE-----''')
            ...
            testlib.config_replace('/etc/racoon/hostacert.pem',
'''-----BEGIN CERTIFICATE-----
...
-----END CERTIFICATE-----''')
            ...
            testlib.config_replace('/etc/racoon/hostakey.pem',
'''-----BEGIN PRIVATE KEY-----
...
-----END PRIVATE KEY-----''')
        ...
        # Start daemons
        self.assertShellExitEquals(0,['/etc/init.d/setkey','start'])
        self.assertShellExitEquals(0,['/etc/init.d/racoon','start'])
...
if __name__ == '__main__':
    ...
    try:
        ...
        if test_type != "psk" and test_type != "cert" and test_type != "nat-t":
            raise
        ...
        print >>sys.stderr, "Usage: %s psk|cert|nat-t LOCAL_IP LOCAL_NET/MASK REMOTE_IP REMOTE_NET/MASK [OPTIONS]" % (sys.argv[0])
        sys.exit(1)
