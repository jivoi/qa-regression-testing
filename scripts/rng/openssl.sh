...
args="$KEYTYPE:$KEYSIZE"
grep_args="Modulus="
if [ "$KEYTYPE" = "dsa" ]; then
    grep_args="Public Key="
    openssl dsaparam -genkey -out "$dsaparam" $KEYSIZE 2>/dev/null
    args="$KEYTYPE:$dsaparam"
fi

for i in $(seq 1 $TRIES) ; do
    echo "$KEYTYPE: $i" > "$status"
    openssl req -newkey "$args" -x509 -nodes -keyout "$keyfile" -subj '/C=US/ST=Arizona/O=Testlib/OU=Test/CN=CA' -out "${keyfile}.out" 2>/dev/null
    openssl "$KEYTYPE" -modulus -in "$keyfile" 2>/dev/null | grep "$grep_args" | cut -d '=' -f 2 | sha1sum | cut -d ' ' -f 1 >> "$hashes"
    rm -f "$keyfile" "${keyfile}.out"
