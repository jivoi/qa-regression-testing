...
''', append=False)
    ...
    def _start(self):
        ...
        for initscript in self.initscript:
            rc, report = testlib.cmd([initscript, 'start'])
            expected = 0
    ...
    def _stop(self):
        '''Stop NUT'''
        for initscript in self.initscript:
            rc, report = testlib.cmd([initscript, 'stop'])
            expected = 0
            result = 'Got exit code %d, expected %d\n' % (rc, expected)
            self.assertEquals(expected, rc, result + report)
    ...
    def _reload(self):
        '''Reload NUT'''
        for initscript in self.initscript:
            rc, report = testlib.cmd([initscript, 'force-reload'])
            expected = 0
            result = 'Got exit code %d, expected %d\n' % (rc, expected)
            self.assertEquals(expected, rc, result + report)
    ...
    def _status(self):
        '''NUT Status'''
        for initscript in self.initscript:
            rc, report = testlib.cmd([initscript, 'status'])
            expected = 0
            if self.lsb_release['Release'] <= 8.04:
