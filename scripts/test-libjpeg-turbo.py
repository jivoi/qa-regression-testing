#!/usr/bin/python
#
#    test-libjpeg-turbo.py quality assurance test script for libjpeg-turbo
...
# QRT-Packages: libjpeg-turbo-progs
...
use_private = True
try:
    from private.qrt.libjpegturbo import LibjpegTurboPrivateTest
except ImportError:
    use_private = False
    print >>sys.stdout, "Skipping private tests"


class LibjpegTurboTest(testlib.TestlibCase):
    '''Test libjpeg-turbo.'''

    def setUp(self):
        '''Set up prior to each test_* function'''
        self.tmpdir = tempfile.mkdtemp(dir='/tmp',prefix="libjpegturbo-")
    ...
    def test_djpeg(self):
        ...
        samples = (
                    ...
                    ('.pnm', '-pnm', r'Netpbm PPM "rawbits" image data(, size = 80 x 72)?')
                  )
...
class LibjpegTurboTestSuite(testlib.TestlibCase):
    '''Testsuite for libjpeg-turbo.'''
    ...
    def test_testsuite(self):
        ...
        build_dir = testlib.prepare_source('libjpeg-turbo', \
                                      self.builder, \
                                      self.cached_src, \
...
if __name__ == '__main__':
    ...
    suite.addTest(unittest.TestLoader().loadTestsFromTestCase(LibjpegTurboTest))
    suite.addTest(unittest.TestLoader().loadTestsFromTestCase(LibjpegTurboTestSuite))

    # Pull in private tests
    if use_private:
        suite.addTest(unittest.TestLoader().loadTestsFromTestCase(LibjpegTurboPrivateTest))
