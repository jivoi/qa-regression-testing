...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
 ...
 <xsl:attribute-set name="properties">
   <xsl:attribute name="margin-top">
     <xsl:if test="position() = 1">-1cm</xsl:if>
     <xsl:if test="position() = 2">1cm</xsl:if>
   </xsl:attribute>
   ...
   <xsl:attribute name="padding-top">
     <xsl:if test="position() = 1">1cm</xsl:if>
     <xsl:if test="position() = 2">0</xsl:if>
   </xsl:attribute>
   <xsl:attribute name="padding-bottom">
     <xsl:if test="position() = 1">0</xsl:if>
   </xsl:attribute>
   <xsl:attribute name="width">
     <xsl:if test="position() = 2">50%</xsl:if>
   </xsl:attribute> 
